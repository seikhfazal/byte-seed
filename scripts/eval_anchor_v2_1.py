...
def stack_pass(answer: str) -> bool:
    lower = answer.lower().strip()
    return bool(lower) and lower != "." and "lifo" in lower and "bfs" not in lower
...
def passes(prompt: str, answer: str) -> bool:
    ...
    if prompt == "Help me plan a 1 hour DSA study session.":
        return "minute" in lower
    if "stack" in prompt.lower():
        return stack_pass(answer)
    if prompt == "What is a queue?":
        return "fifo" in lower and "enqueue" in lower and "dequeue" in lower
    if prompt == "What is overfitting?":
        return "training" in lower and ("validation" in lower or has_any(lower, ("unseen", "new data")))
    ...
    if prompt == "Should I upload checkpoints to GitHub?":
        return ("do not commit" in lower or "avoid committing checkpoints" in lower) and "checkpoint" in lower
    return bool(answer)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Evaluate Anchor v2.1 checkpoint on patch target prompts.")
    parser.add_argument("--config", default="configs/byteseed_12m.yaml")
    parser.add_argument("--checkpoint", default="checkpoints/anchor_v2_1_finetuned.pt")
